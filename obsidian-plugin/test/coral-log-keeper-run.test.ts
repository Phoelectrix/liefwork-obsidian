...
/** Run the keeper the way a hook fires from a MOVED session cwd: the script
 *  addressed absolutely, the process working directory somewhere else entirely
 *  (a branch folder inside the coral). Returns the raw process result — the
 *  caller asserts where the state landed. */
async function runFromCwd(dir: string, cwd: string) {
  const proc = Bun.spawn(["node", join(dir, ".claude/hooks/coral-log-keeper.mjs")], {
    cwd,
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  await proc.exited;
  return { out, stderr, code: proc.exitCode };
}
...
// ── cwd independence ────────────────────────────────────────────────────────
// A Claude Code session's working directory MOVES as the work moves (5 Aug: it
// walked from a coral root down into a branch folder, then a sub-branch). The
// keeper is anchored to the coral by the script's own location, never by cwd —
// so it reads and writes the ROOT's state and scans the WHOLE coral from
// wherever it happens to be run.

test("run from a branch folder: state is written at the CORAL ROOT, not in the cwd", async () => {
  const dir = makeCoral({ recordingPref: "default", counter: 1, lastLiefMtime: 0, threshold: 4 });
  const branch = join(dir, "Larry Sultan", "Corpus");
  mkdirSync(branch, { recursive: true });

  const { code, stderr } = await runFromCwd(dir, branch);
  expect(stderr).toBe("");
  expect(code).toBe(0);

  const rootState = JSON.parse(readFileSync(join(dir, ".claude", ".coral-log-state.json"), "utf8"));
  expect(rootState.counter).toBe(2);
  // No stray state file seeded inside the branch the session happened to be in.
  expect(existsSync(join(branch, ".claude", ".coral-log-state.json"))).toBe(false);
});

test("run from a branch folder: a lief ELSEWHERE in the coral still resets the counter", async () => {
  // The discriminant for the scan root: the lief lives in a sibling branch the
  // cwd cannot see. A cwd-relative scan finds nothing and increments; a
  // coral-rooted scan finds it and resets to 0.
  const dir = makeCoral({ recordingPref: "default", counter: 3, lastLiefMtime: 0, lastDirMtimeMax: 0, threshold: 4 });
  const branch = join(dir, "Larry Sultan");
  mkdirSync(branch, { recursive: true });
  writeLief(dir, "Other Branch/prompt — hello.md");

  const { code, out } = await runFromCwd(dir, branch);
  expect(code).toBe(0);
  expect(out.trim()).toBe("");

  const rootState = JSON.parse(readFileSync(join(dir, ".claude", ".coral-log-state.json"), "utf8"));
  expect(rootState.counter).toBe(0);
});
