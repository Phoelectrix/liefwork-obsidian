...
test("planSeed: a sub-coral gets a root-meristem + a founding lief, and inherits (no CLAUDE.md)", () => {
  const files = planSeed({
  ...
  expect(paths).toEqual([
    "Launch RoadMap/Launch RoadMap.md", // root meristem (folder-note)
    "Launch RoadMap/prompt — founding intent.md", // founding lief
  ]);
  // a sub-coral inherits the parent Charter — no markers of its own
  for (const marker of ["Growth Charter.md", "CLAUDE.md", "AGENTS.md", "GEMINI.md"]) {
    expect(paths.some((p) => p.endsWith(marker))).toBe(false);
  }
});
...
test("planSeed: a sub-coral gets NO keeper (inherits the parent root's)", () => {
  const files = planSeed({
    folderPath: "Foo/Sub",
    name: "Sub",
    description: "",
...
test("readCoralFacts: a folder with Growth Charter.md reads hasOwnCharter", () => {
  ...
  expect(facts.hasOwnClaude).toBe(false);
});
