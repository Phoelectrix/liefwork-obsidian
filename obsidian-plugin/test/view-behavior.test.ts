...
// A theme change swaps the styling the mount was CREATED with; mount.update()
// keeps styling, so refresh() would leave the coral wearing the old palette
// (found live, 28 Aug 2026). Structural settings rebuild the plant in place.
test("settingRefreshMode: theme remounts, structural settings update in place", () => {
  expect(settingRefreshMode("theme")).toBe("remount");
  expect(settingRefreshMode("showMeristemNotes")).toBe("update");
  expect(settingRefreshMode("allFiles")).toBe("update");
});
