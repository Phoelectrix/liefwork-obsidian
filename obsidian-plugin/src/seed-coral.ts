...
export function planSeed(opts: SeedOptions): SeedFile[] {
  ...
  // already-governed folder that produced no Charter read as a broken command.)
  {
    ...
    // episode-floor logging from decaying. EVERY seed plants its own, sub-corals
    // included: "sub-corals inherit it" (what this comment used to say) is false
    // and was the wrong mental model in the one place it mattered — Claude Code
    // reads `.claude/settings.json` ONLY from the directory a session was started
    // in, never a parent's and never a child's (verified 5 Aug). So a hook is
    // live for sessions started at ITS OWN coral root and nowhere else; a seeded
    // subfolder is its own root, with its own reminder and its own count. The
    // Charter, by contrast, IS inherited down the tree — the rules reach an agent
    // started anywhere; only the nudge doesn't. Documented in the Charter + README.
