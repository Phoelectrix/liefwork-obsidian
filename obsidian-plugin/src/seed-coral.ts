...
export interface SeedOptions {
  ...
  /** True when nested in an existing coral (inherits the parent Growth Charter). */
  isSubCoral: boolean;
...
export function planSeed(opts: SeedOptions): SeedFile[] {
  ...
  // A root coral carries its own canonical Growth Charter (+ generated vendor
  // pointers redirecting each agent's native auto-load to it); a sub-coral
  // inherits the parent's via the up-tree walk, so it gets none of its own.
  if (!opts.isSubCoral) {
    files.push({
