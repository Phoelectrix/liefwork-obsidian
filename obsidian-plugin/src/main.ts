...
import { classifyFolder, coralMenuActions, planUndo, planReactivate } from "./coral-lifecycle.ts";
import { GROWTH_CHARTER_BASENAME, AGENT_POINTER_BASENAMES } from "./seed-templates.ts";
import { unmergeKeeperHook } from "./settings-merge.ts";
...
export default class LiefworkPlugin extends Plugin {
  ...
  async onload() {
    ...
    // lifecycle items (the Guard: an already-seeded folder no longer offers a
    // silent re-seed — it offers Undo / Reactivate / Re-seed as applicable).
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file: TAbstractFile) => {
        ...
        for (const action of coralMenuActions(status)) {
          if (action === "seed") {
            ...
            menu.addItem((item) =>
              item
                .setTitle("Undo seeding")
                .setIcon("sprout")
                .onClick(() => { void this.runLifecycle(file, "undo"); }),
            );
            ...
            menu.addItem((item) =>
              item
                .setTitle("Re-seed coral…")
                .setIcon("sprout")
                .onClick(() => {
                  new SeedCoralModal(this.app, file, (p) => void this.openPlant(p), "reseed").open();
                }),
            );
          }
        }
   ...
   *  Undo Seeding archives ALL the seed documents (not data) into a dated
   *  `_archive — unseeded (…)/` with a provenance log; Reactivate restores them.
   *  Nothing is ever deleted. */
  private async runLifecycle(folder: TFolder, kind: "undo" | "reactivate"): Promise<void> {
    ...
    try {
      if (kind === "undo") {
        ...
        // The keeper script's presence drives `hasKeeper` (whether `planUndo`
        ...
        const plan = planUndo({
          ...
          markerNames: [GROWTH_CHARTER_BASENAME, ...AGENT_POINTER_BASENAMES].filter((n) => exists(n)),
          hasFoundingLief: exists("prompt — founding intent.md"),
          hasKeeper,
          today: todayISO(),
        });
        await applyArchivePlan(this.app, plan);
        // settings.json isn't moved by planUndo: if we created it, archive the
          ...
          if (await adapter.exists(settingsPath)) {
            if (settingsCreatedByUs) {
              ...
              try {
                ...
                // the undo already succeeded.  Tell the user to remove the hook manually.
                new Notice(
        ...
        new Notice(`Liefwork: un-seeded “${folder.name}” — seed docs archived (Reactivate to restore)`);
      } else {
        const archive = unseedArchives(folder)[0];
        if (!archive) {
        ...
        const plan = planReactivate({
          ...
          meristemName: `${folder.name || this.app.vault.getName()}.md`,
          archivedNames: archive.children.map((c) => c.name),
        });
