...
import { resolveTheme, toggleTheme, type CoralTheme } from "./theme.ts";
import { resolveStyle, type CoralStyle } from "./style.ts";
...
interface LiefworkPluginData {
  ...
  /** Coral colour theme (see theme.ts). Absent = dark. Free — light mode is
   *  the friendliness feature, not a Pro perk. */
  theme?: CoralTheme;
  /** Coral branching style (see style.ts). Absent = default, the shipped
   *  uniform fan; "organic" = wider first angle with capped decay. Structural. */
  style?: CoralStyle;
}
...
export default class LiefworkPlugin extends Plugin {
  ...
  /** Coral colour theme (theme.ts). A change rebuilds every open coral in the
   *  other palette — rerender() keeps the camera, so it lands in the same view. */
  private theme: CoralTheme = "dark";

  getTheme(): CoralTheme { return this.theme; }

  async setTheme(t: CoralTheme): Promise<void> {
    this.theme = t;
    const data = ((await this.loadData()) ?? {}) as LiefworkPluginData;
    data.theme = t;
    await this.saveData(data);
    // A look change, not a structural one: remount (settingRefreshMode).
    for (const leaf of this.app.workspace.getLeavesOfType(LIEFWORK_VIEW_TYPE)) {
      if (leaf.view instanceof PlantView) leaf.view.remount();
    }
  }

  /** Coral branching style (style.ts). Structural: a change re-runs the
   *  layout engine for every open coral (refresh keeps the camera). */
  private style: CoralStyle = "default";

  getStyle(): CoralStyle { return this.style; }

  async setStyle(s: CoralStyle): Promise<void> {
    this.style = s;
    const data = ((await this.loadData()) ?? {}) as LiefworkPluginData;
    data.style = s;
    await this.saveData(data);
  ...
  async onload() {
    ...
    this.theme = resolveTheme(data.theme);
    ...
    // The light/dark A/B: one keystroke, same camera, instant comparison.
    this.addCommand({
      id: "toggle-theme",
      name: "Toggle light/dark coral",
      callback: () => { void this.setTheme(toggleTheme(this.theme)); },
    });
