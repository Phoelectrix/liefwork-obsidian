...
import { KEEPER_COMMAND } from "./settings-merge.ts";
 ...
 */
export const CORAL_LOG_KEEPER_SCRIPT = `import { readFileSync, writeFileSync, readdirSync, statSync, realpathSync } from "node:fs";
import { join, dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";

// The coral is located from THIS FILE, never from the working directory. A hook
// runs in the SESSION's cwd, and that moves as the work moves (5 Aug: a session
// walked from a coral root down into a branch folder, then a sub-branch).
// Anchoring on cwd meant reading a state file that wasn't there, scanning a
// fraction of the coral, and writing a stray state file into whichever folder
// the agent had wandered into. This script always lives at
// <coral>/.claude/hooks/, so two levels up is the coral root — true wherever
// the process happens to be started.
const CORAL_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");

const STATE_PATH = join(CORAL_ROOT, ".claude", ".coral-log-state.json");
...
function main() {
  try {
    ...
    const curDirMtime = maxDirMtime(CORAL_ROOT);
    let newestLiefMtime;
    if (curDirMtime > (state.lastDirMtimeMax ?? 0)) {
      newestLiefMtime = scanNewestLiefMtime(CORAL_ROOT);
    } else {
...
// Entry-point check by REAL path on both sides. \`import.meta.url\` is always
// fully resolved while argv[1] is the path as typed, so a string compare fails
// wherever the coral is reached through a symlink (/tmp, a synced-folder alias,
// a home-directory link) — and the hook would then run, exit 0 and do nothing
// at all, silently. Falls back to the plain compare if realpath can't resolve.
function isEntryPoint(scriptPath, argvPath) {
...
if (isEntryPoint(fileURLToPath(import.meta.url), process.argv[1])) main();
`;

/** `.claude/settings.json` registering the keeper on UserPromptSubmit. The
 *  command comes from `settings-merge.ts` so the greenfield file and the
 *  brownfield merge can never drift apart (they did: this literal was a second
 *  copy of it). */
export const CORAL_SETTINGS_JSON = JSON.stringify(
  {
    hooks: {
      UserPromptSubmit: [{ hooks: [{ type: "command", command: KEEPER_COMMAND }] }],
    },
  },
  null,
  2,
...
## Who keeps these rules

**This document is the mechanism.** There is no enforcement underneath it: a coral is worth exactly what the recording and pruning put into it, by the agent and by the person working with it. The two moments that matter are the **start of a session** (settle the recording preference — ritual step 3) and the **end of a piece of work** (write the \`prompt\` + \`response\` pair and re-summarise the branch above them, before moving on).

**Agents with hook support** can carry a *reminder* alongside these rules: a small local script, installed at seeding, that counts turns since the last lief was written and nudges when an episode has clearly ended. It runs only for sessions **started in the folder holding this Charter**, since that is the one place such an agent reads its hook configuration from — a folder seeded *inside* this coral is its own root, with its own reminder and its own count. An agent started deeper in the tree still reads this Charter (instructions are inherited down the tree; the reminder is not), and every other agent works to this document alone. **The recording is the agent's job either way — the reminder only helps it notice.**
`;
