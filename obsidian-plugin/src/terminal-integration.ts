...
/** What "Open agent terminal here" should do, once the environment has
 *  answered. Its whole job is the invariant **never bind without a terminal**:
 *  a binding tints its branch and pips the coral, and the ONLY thing that ends
 *  one is its terminal leaf closing (main.ts's layout-change sweep), so a
 *  leafless binding is a tint the user cannot clear. The command explains
 *  itself instead — and, for a missing plugin, explains BEFORE the colour
 *  prompt, so nothing is asked for that can't be delivered. */
export type AgentTerminalOutcome =
  | { kind: "explain-missing-plugin" }
  | { kind: "explain-launch-failed" }
  | { kind: "bind"; launchedVia: LaunchedVia };

type LaunchedVia = "integrated";

export function agentTerminalOutcome(input: {
  pluginAvailable: boolean;
  /** The leaf `openIntegratedTerminal` produced — null/undefined if none. */
  leaf: unknown;
}): AgentTerminalOutcome {
  // The leaf IS the terminal: if one exists there is something to bind to and
  // something to close, whatever the availability probe read.
  if (input.leaf) return { kind: "bind", launchedVia: "integrated" };
  return input.pluginAvailable
    ? { kind: "explain-launch-failed" }
    : { kind: "explain-missing-plugin" };
}
