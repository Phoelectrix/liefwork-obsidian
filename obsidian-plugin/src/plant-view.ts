...
import { promptForName, confirm } from "./name-prompt-modal.ts";
...
import { openIntegratedTerminal } from "./terminal-integration.ts";
import { clearTabDeco } from "./terminal-tab-deco.ts";
...
import { coralEngineConfig } from "./engine-config.ts";
import {
...
export class PlantView extends ItemView {
   ...
   *  rebuild — so Seed/Undo/Re-seed (which all trigger a rebuild) show/hide
  ...
  private showNodeMenu(node: NodeRef, evt: MouseEvent): void {
    ...
    if (node.kind === "lief") {
      ...
      const agentBindings = this.store.bindingsForBranch(agentBranchPath);
      if (agentBindings.length > 0) {
        // Jump to the agent's terminal (the first one launched here, if several).
        const firstWithLeaf = agentBindings.find((b) => b.leaf);
        if (firstWithLeaf) {
          menu.addItem((i) =>
        ...
        menu.addItem((i) =>
          i.setTitle("Stop agent terminal").setIcon("square").onClick(() => {
            for (const b of this.store.bindingsForBranch(agentBranchPath)) {
              if (b.leaf) clearTabDeco(b.leaf as WorkspaceLeaf);
              this.store.endById(b.id);
            }
          }),
        );
      }
  ...
  private async openAgentTerminal(node: NodeRef): Promise<void> {
    ...
    this.store.create({
      ...
      color,
      leaf: leaf ?? undefined,
      launchedVia: leaf ? "integrated" : "manual",
    });
    if (!leaf) {
      new Notice(`Liefwork: run your agent in ${branchPath || "the vault root"} (no integrated terminal available)`);
    }
  }
  ...
  toggleDevPanel(): boolean {
    ...
    this.devPanel = installDevPanel(this.mount, {
      ...
      showEngineKnobs: false, // no live engine config in the plugin mount
    });
    return true;
  }
  ...
  refresh(): void {
    ...
    if (this.mount) {
      ...
      // Seed/Undo/Re-seed's rebuild takes, so the badge appears/disappears
