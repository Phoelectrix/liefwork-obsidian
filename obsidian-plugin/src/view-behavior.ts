...
/** How an open coral takes a settings change. Structural settings (what the
 *  tree contains) rebuild the plant in place via mount.update(), which keeps
 *  the camera AND the styling the mount was created with. A theme change is
 *  a styling change, so update() would leave the coral wearing the old
 *  palette — it needs a fresh mount (rerender also restores the camera). */
export type SettingRefreshMode = "remount" | "update";
export function settingRefreshMode(
  setting: "theme" | "showMeristemNotes" | "allFiles",
): SettingRefreshMode {
  return setting === "theme" ? "remount" : "update";
}
