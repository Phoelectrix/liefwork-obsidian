import { subtreeNormByBranch } from "./subtree-weight.ts";
...
export interface DevKnobs {
  ...
  /** projection.meristemLiftEm — screen-space title lift; redraw only. */
  meristemLiftEm: number;
  /** Text presence (palette-carried in palettes.ts; live here for tuning).
   *  labelWeight is the CSS font-weight as a NUMBER (100–900) — StylingConfig
   *  keeps it as a string; the knob converts. */
  labelWeight: number;
  dotGlyphRadiusEm: number;
  labelStrokePx: number;
  /** projection.lightOpacity — label alpha. */
  lightOpacity: number;
  ...
  branchFitPad: number;
  /** camera.zoomOutMax — how far past Full View the camera may pull back, as a
   *  multiple of the full-view frame. */
  zoomOutMax: number;
  /** camera.pinchZoomGain — how much harder a trackpad pinch zooms than a
   *  two-finger scroll of the same delta. */
...
export interface EngineKnobs {
  ...
  childScale: number;
  /** `angles.firstBranchAngle` — the seed: the root fan's first departure, degrees. */
  firstBranchAngle: number;
  /** `angles.angleDecay` — per-side sibling decay (1 = every sibling at the seed). */
  angleDecay: number;
  /** `angles.minBranchAngle` — the floor under the decay, degrees (0 = off). */
  minBranchAngle: number;
  /** `clearance.childAxisGrowthBlockFactor` — the dead zone at the start of
   *  every child chain (× S), before its first block. */
  childAxisGrowthBlockFactor: number;
  /** `clearance.meristemInFactor` / `meristemOutFactor` — the weight-
   *  proportional clearance before / after every meristem (× S × weight). */
...
export interface PlantMountHandle {
   ...
   *  camera — host framing and scripted moves don't count. Hosts use this to
   *  skip camera moves the reader didn't ask for; the pan-zoom layer already
   *  preserves an owned camera through pane reflows. */
...
export function mountPlant(
  ...
  const panzoom: PanZoomHandle = installPanZoom(
    ...
    (tx, ty, scale) => skeleton.redraw(tx, ty, scale),
    {
      ...
      // Reserve the root title's height + its worst-case screen lift (read
      // live, so the dev-panel knob refits correctly).
      extraInsetPx: () => fitLabelInsetPx(styling.projection.meristemLiftEm),
      zoomOutMax: styling.camera.zoomOutMax,
      pinchZoomGain: styling.camera.pinchZoomGain,
  ...
  return {
    ...
    isCameraUserOwned() {
      return panzoom.isCameraUserOwned();
    },
    ...
    setDevKnobs(k) {
      ...
      if (k.meristemDistanceWeightGain !== undefined) { styling.projection.meristemDistanceWeightGain = k.meristemDistanceWeightGain; structural = true; }
      if (k.meristemLiftEm !== undefined) { styling.projection.meristemLiftEm = k.meristemLiftEm; needsRedraw = true; }
      // Text presence. Weight is structural: the block measurer + hitTest go
      // through labelFont(styling.labelWeight), so label layout must rebuild.
      if (k.labelWeight !== undefined) { styling.labelWeight = String(k.labelWeight); structural = true; }
      if (k.dotGlyphRadiusEm !== undefined) { styling.dotGlyphRadiusEm = k.dotGlyphRadiusEm; needsRedraw = true; }
      if (k.labelStrokePx !== undefined) { styling.labelStrokePx = k.labelStrokePx; needsRedraw = true; }
      if (k.lightOpacity !== undefined) { styling.projection.lightOpacity = k.lightOpacity; needsRedraw = true; }
      if (structural) {
        currentProjConfigMap = buildProjectionConfigMap(plant, styling);
        skeleton.setPlant(plant, undefined, currentProjConfigMap, null, new Set());
        if (cachedTintColorsByPath.size > 0) applyAgentTintsInternal(cachedTintColorsByPath);
      ...
      // zoomOutMax is the one camera knob that bites AT ONCE (setZoomOutMax
      // re-clamps), so dragging the slider shows the new pull-back range on the
      // coral in front of you rather than at the next fit.
      if (k.zoomOutMax !== undefined) {
        styling.camera.zoomOutMax = k.zoomOutMax;
        panzoom.setZoomOutMax(k.zoomOutMax);
      }
      if (k.pinchZoomGain !== undefined) {
        styling.camera.pinchZoomGain = k.pinchZoomGain;
        panzoom.setPinchZoomGain(k.pinchZoomGain);
      }
    ...
    getDevKnobs() {
      return {
        ...
        meristemDistanceWeightGain: styling.projection.meristemDistanceWeightGain,
        meristemLiftEm: styling.projection.meristemLiftEm,
        labelWeight: Number(styling.labelWeight) || 400,
        dotGlyphRadiusEm: styling.dotGlyphRadiusEm,
        labelStrokePx: styling.labelStrokePx,
        lightOpacity: styling.projection.lightOpacity,
    ...
    setEngineKnobs(k) {
      ...
      if (k.firstBranchAngle !== undefined) engineConfig.angles.firstBranchAngle = k.firstBranchAngle;
      if (k.angleDecay !== undefined) engineConfig.angles.angleDecay = k.angleDecay;
      if (k.minBranchAngle !== undefined) engineConfig.angles.minBranchAngle = k.minBranchAngle;
      if (k.childAxisGrowthBlockFactor !== undefined) engineConfig.clearance.childAxisGrowthBlockFactor = k.childAxisGrowthBlockFactor;
      if (k.meristemInFactor !== undefined) engineConfig.clearance.meristemInFactor = k.meristemInFactor;
      if (k.meristemOutFactor !== undefined) engineConfig.clearance.meristemOutFactor = k.meristemOutFactor;
    ...
    getEngineKnobs() {
      return {
        ...
        firstBranchAngle: engineConfig?.angles.firstBranchAngle ?? 60,
        angleDecay: engineConfig?.angles.angleDecay ?? 0.95,
        minBranchAngle: engineConfig?.angles.minBranchAngle ?? 0,
        childAxisGrowthBlockFactor: engineConfig?.clearance.childAxisGrowthBlockFactor ?? 2,
        meristemInFactor: engineConfig?.clearance.meristemInFactor ?? 1,
        meristemOutFactor: engineConfig?.clearance.meristemOutFactor ?? 1,
      };
    },
...
export function buildProjectionConfigMap(
  ...
  const subtreeNorm = subtreeNormByBranch(plant);
  ...
  for (const b of plant.blocks) {
    ...
    let meristemDistance =
      b.kind === "meristem" && b.id === rootMeristemBlockId
        ? cfg.rootMeristemDistance
        : cfg.meristemDistance;
    // Weight-proportional lift (see styling.ts meristemDistanceWeightGain).
    if (b.kind === "meristem" && cfg.meristemDistanceWeightGain > 0) {
      meristemDistance *= 1 + cfg.meristemDistanceWeightGain * (subtreeNorm.get(b.branchId) ?? 0);
    }
