...
/**
 * Breathing room around a framed branch, as a fraction of the viewport.
 *
 * This was **0.42** until 28 July, and it was compensating for a bug rather
 * than composing a shot: `frameBranch` resolved to a fixed 60x60 box
 * synthesised around a block's position, so only enormous padding kept the
 * target on screen at all. With `branch-target.ts` handing back the branch's
 * real subtree AABB, that padding shrank the branch to a fraction of the frame
 * — the two faults had been cancelling, and fixing one alone looked like no
 * fix. Kept non-zero because the AABB unions block positions, and labels hang
 * outside them.
 */
const FRAME_BRANCH_PADDING = 0.12;
...
function resolveKeyframe(kf: Keyframe, ctx: ResolveCtx): Transform {
  ...
  return computeFit(b, ctx.viewport, FRAME_BRANCH_PADDING);
}
