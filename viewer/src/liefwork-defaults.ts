...
import { meridian } from "./palettes.ts";
...
export const liefworkStylingDefaults = {
  ...
  projection: {
    ...
    liefDistance: 1.8,
    meristemDistance: 3,
    meristemDistanceWeightGain: meridian.meristemDistanceWeightGain,
    meristemLiftEm: meridian.meristemLiftEm,
    titleSize: 0.24,
    light: meridian.liefLabel, // Meridian: Star White (lief labels)
    meristemLight: meridian.meristemLabel, // Meridian: Amber Window (folder/branch hierarchy — pale straw-gold)
    pathLight: meridian.pathLabel, // Meridian: Silver Facade (path-to-root — neutral; teal reserved for status)
    ...
    sourceOpacity: 0,
    ray: {
      stroke: meridian.ray, // Meridian: Dusty Periwinkle (subtle connector rays)
      opacity: 0.35,
      width: 1,
    },
    showBounds: false,
  ...
  body: {
    fill: meridian.bodyFill, // Meridian: Deep Slate mid-tone fill (currently off — baseOpacity 0)
    baseOpacity: 0,
    cursorBoost: 1.6,
    edgeSoftness: 0.5,
  },
  ...
  background: {
    color: meridian.background, // Meridian: Void
  },
  ...
  bubbleWidth: 0.5,
  subtreeBoost: 0.65,
  subtreeBoostShape: meridian.subtreeBoostShape, // 3.8 — rides the palette since 22 Sept 2026
  ...
  liefDotsAbovePx: meridian.liefDotsAbovePx, // 1 — rides the palette since 22 Sept 2026
  liefFullAbovePx: 5.5,
  liefPeakSize: 100,
  ...
  panelTheme: meridian.panelTheme,
  // Extracted from skeleton-canvas constants on 28 Aug 2026 (light-mode work) —
  // same values, now palette roles.
  selectedLabelColor: meridian.selectedLabel,
  fileLiefColor: meridian.fileLief,
  liefBlockFill: meridian.liefBlockFill,
  labelWeight: meridian.labelWeight,
  dotGlyphRadiusEm: meridian.dotGlyphRadiusEm,
  labelStrokePx: meridian.labelStrokePx,
  liefBlock: {
    mode: "label",
    justify: true,
    sizeRatio: 0.58,
