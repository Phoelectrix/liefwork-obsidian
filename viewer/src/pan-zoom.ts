...
export interface PanZoomOptions {
   ...
   *  behaviour change. */
  fitPadding?: number;
  /** Extra viewport inset in CSS px on every side, read at each fit — the
   *  host reserves room for screen-space label offsets the world bounds can't
   *  express (the meristem screen lift, skeleton-canvas meristemLiftInsetPx). */
  extraInsetPx?: () => number;
...
export function installPanZoom(
  ...
  const readInsets = (svgRect: { left: number; right: number }): Insets => {
    const extra = opts?.extraInsetPx?.() ?? 0;
    const insets: Insets = extra > 0 ? { top: extra, right: extra, bottom: extra, left: extra } : {};
    const studio = document.getElementById("studio");
    if (studio) {
      const r = studio.getBoundingClientRect();
      insets.right = Math.max(0, svgRect.right - r.left) + CHROME_MARGIN_PX + extra;
    }
    const hud = document.getElementById("hud");
    if (hud) {
      const r = hud.getBoundingClientRect();
      insets.left = Math.max(0, r.right - svgRect.left) + CHROME_MARGIN_PX + extra;
    }
    ...
    if (document.body.classList.contains("panel-inset")) {
      ...
      if (panel && !panel.classList.contains("size-square")) {
        ...
        if (r.width > 0 && r.left > 0) {
          insets.right = Math.max(insets.right ?? 0, r.width * 1.08 + CHROME_MARGIN_PX + extra);
        }
      }
    }
    return insets;
