...
export interface StemConfig {
  ...
  tipOpacity: number;
  subtreeExp: number;
  /** Minimum stem width in SCREEN px — the zoom-out floor. A palette value
   *  (light mode lifts it: 1.5 px is exactly where a dark hairline gets eaten
   *  by a white field), so it must not be a canvas constant. */
  minScreenPx: number;
}
...
export interface ProjectionConfig {
  ...
  liefDistance: number;
  /** Weight-proportional lift of the meristem ray: effective ray =
   *  meristemDistance × (1 + gain × subtreeNorm), subtreeNorm ∈ [0, 1] being
   *  the branch's log-normalised subtree weight (subtree-weight.ts; the root
   *  is 1). 0 = today's fixed rays. A heavy branch's title lifts clear of the
   *  children crowding its tip; a young coral barely moves — which is what
   *  the fixed 50-unit root stalk of 14 July could not do. Applies to the root
   *  on top of rootMeristemDistance. */
  meristemDistanceWeightGain: number;
  /** SCREEN-space lift of a meristem title along its ray, in px =
   *  meristemLiftEm × fontPx × subtreeNorm. The world-unit ray shrinks with
   *  the zoom while titles hold their screen size, so zoomed out they pile up
   *  whatever the world lift; this one keeps pace with the title itself.
   *  Drawn with a short screen-space ray extension so the title stays
   *  visibly tethered. 0 = off. */
...
/** Camera framing: how much of the window a fitted/framed branch fills, and how
 *  far the reader may pull back from it. Config-gated house-style knobs over
 *  previously-hardcoded constants. See viewer/src/fit.ts (computeFitTransform,
 *  minZoomForFit) and mount-plant.ts's frameNodeInternal / frameNodeByNodeId. */
export interface CameraConfig {
   ...
   *  full-view frame: 1 = no further than Full View, 1.25 = the 25% empty margin
   *  that was hardcoded until 5 Aug 2026, 3 = three times the frame. The zoom
   *  floor is `fitScale / zoomOutMax`. Values below 1 are refused — a floor
   *  above the fit would put Full View itself out of reach.
   *
   *  Shipped default **2**, founder-tuned: on a WIDE coral with long meristem
   *  labels the text ran out of frame and could not be pulled back into view.
   *  A fit cannot solve that, because label size does not scale with zoom —
   *  `computeTargetScreenPx` lifts each label from its basal (zoom-proportional)
   *  size toward a fixed screen-px optimum, so zooming out shrinks the geometry
   *  while the labels keep their screen size. Any label-inclusive bounds is
   *  therefore only true at the zoom it was measured at, and headroom is the
   *  honest instrument. */
  zoomOutMax: number;
  /** How much harder a trackpad PINCH zooms than a two-finger scroll of the
   *  same reported delta. 1 = identical (the behaviour before 5 Aug 2026).
   *
   *  A pinch produces no touch or pointer events — the OS delivers it as a
   *  wheel event with `ctrlKey` set — and it carries much smaller deltas for
   *  the same physical travel, so sharing one sensitivity leaves the pinch
   *  feeling dead. This gains up the pinch delta alone; a plain scroll is
   *  untouched. Values below 1 are refused: a pinch is never duller than the
   *  gesture it is meant to feel more direct than.
   *
   *  Shipped default **7**, founder-tuned on the trackpad. High, and correctly
   *  so — it is the ratio between what the OS reports for a pinch and for a
   *  scroll of the same physical travel, not a taste setting. */
...
export interface StylingConfig {
  ...
  camera: CameraConfig;
  /** The selected node's label colour. Was a skeleton-canvas constant
   *  (Meridian Peach Rose) until the light-mode palette work (28 Aug 2026)
   *  made it a palette role. */
  selectedLabelColor: string;
  /** Label colour for a non-markdown file lief (all-files mode). Producer-set
   *  via LiefBlock.nonMd, so only the plugin's file liefs ever pick it up. */
  fileLiefColor: string;
  /** Lief summary-block fill — dim so the block reads as secondary. */
  liefBlockFill: string;
  /** CSS font-weight for every canvas label (draw pass AND hit-test measure —
   *  see skeleton-canvas `labelFont`). Light mode buys back with weight what
   *  halation donated for free on the Void. */
  labelWeight: string;
  /** Dot-glyph tier: dot radius as a fraction of the label's font px. A
   *  hairline-class mark — 0.15 reads on the Void, gets eaten by a white
   *  field — so it rides the palette like stem width and label weight. */
  dotGlyphRadiusEm: number;
  /** Text presence (10 Sept 2026): a same-colour stroke drawn under every
   *  full-tier label, in screen px. 0 = none (the pre-0.2.0 draw). Dark-on-
   *  white small text loses its edges to anti-aliasing against the bright
   *  surround — the mirror of the halation that fattens light-on-dark for
   *  free — and a hairline stroke buys that weight back at one extra draw call
   *  per label. Negligible on large text, decisive at 8–12 px, so it rides
   *  the palette rather than a size ramp. */
...
export const defaultStylingConfig: StylingConfig = {
  ...
  panelTheme: "dark",
  selectedLabelColor: "#BE7355",
  fileLiefColor: "#5ECFBE",
  liefBlockFill: "rgba(238,242,248,0.62)",
  labelWeight: "400",
  dotGlyphRadiusEm: 0.15,
  labelStrokePx: 0,
  atmosphere: {
    horizon: false,
    horizonStrength: 0.5,
...
export function mergeStylingConfig(partial: Partial<StylingConfig>): StylingConfig {
  ...
  return {
    ...
    selectedLabelColor: partial.selectedLabelColor ?? defaultStylingConfig.selectedLabelColor,
    fileLiefColor: partial.fileLiefColor ?? defaultStylingConfig.fileLiefColor,
    liefBlockFill: partial.liefBlockFill ?? defaultStylingConfig.liefBlockFill,
    labelWeight: partial.labelWeight ?? defaultStylingConfig.labelWeight,
    dotGlyphRadiusEm: partial.dotGlyphRadiusEm ?? defaultStylingConfig.dotGlyphRadiusEm,
    labelStrokePx: partial.labelStrokePx ?? defaultStylingConfig.labelStrokePx,
  };
}
