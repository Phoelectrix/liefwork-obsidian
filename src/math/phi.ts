...
export function siblingAngles(
  ...
  if (alternate) {
    ...
    for (let i = 0; i < n; i++) {
      if (i % 2 === 0) {
        out[i] = firstAngleDegrees * Math.pow(decay, leftIdx);
        leftIdx++;
      } else {
        out[i] = -firstAngleDegrees * Math.pow(decay, rightIdx);
        rightIdx++;
      }
    }
  } else {
    for (let i = 0; i < n; i++) out[i] = firstAngleDegrees * Math.pow(decay, i);
  }
