...
import type { GrowthBlockOutput } from "./growth-block.ts";
import { sampleCurve } from "../../curves/sample.ts";

export interface PlaceOutput extends GrowthBlockOutput { bounds: Bounds; }

export function placePass(g: GrowthBlockOutput, _cfg: EngineConfig): PlaceOutput {
  ...
  const byDepthAsc = [...g.branches].sort((a, b) => a.depth - b.depth);

  for (const br of byDepthAsc) {
    let origin: Vec2;
    let originAngle: number;
    if (br.parentBranchId === null) {
      origin = g.anchor.position;
      originAngle = g.anchor.orientationAngle + br.departureAngle;
    } else {
  ...
  // Propagate descendants' bounds up parent chains.
  const childrenOf = new Map<string | null, typeof g.branches[number][]>();
  for (const b of g.branches) {
    const arr = childrenOf.get(b.parentBranchId) ?? [];
    arr.push(b);
    childrenOf.set(b.parentBranchId, arr);
  }
  const byDepthDesc = [...g.branches].sort((a, b) => b.depth - a.depth);
